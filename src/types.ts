...
export interface LocationData { city: string; province: string; country: string; lat: number; lng: number; approxZone: string; }
export interface PrivateUserInfo { phonePrivate?: string; exactAddressPrivate?: string; }
export type InternalPrivateProfileData = { phonePrivate?: string; location: LocationData & { exactAddressPrivate?: string; }; };

export interface UserProfile {
  id: string; name: string; email: string; avatar: string; role: Role; joinedDate: string; location: LocationData;
  isIdentityVerified: boolean; identityVerificationStatus: VerificationStatus; bioPublic?: string; isBlocked?: boolean;
  activeMode?: 'CLIENT' | 'PROFESSIONAL' | 'ADMIN'; hasClientProfile?: boolean; hasProfessionalProfile?: boolean;
  workHours?: string; isProfessional?: boolean; businessName?: string; professionId?: string; professionName?: string;
  specialties?: string[]; description?: string; workZoneRadiusKm?: number; isProfessionalVerified?: boolean;
  professionalVerificationStatus?: VerificationStatus; matriculaOrDegree?: string; rating: number; reviewCount: number;
  jobsCompleted: number; trustScore: number; availabilityStatus: 'DISPONIBLE' | 'OCUPADO' | 'EN_CONSULTA';
  hourlyRateArs?: number; servicesOffered?: ServiceItem[]; portfolioImages?: string[]; workingHours?: string;
  isProSubscriber?: boolean; isFeatured?: boolean; isDemoData?: boolean;
}
...
export interface Transaction {
  ...
  platformFeePercent: number; platformFeeAmountArs: number; professionalAmountArs: number; paymentProcessingFeeArs?: number;
  netPlatformRevenueArs?: number; status: TransactionStatus; paymentStatus?: PaymentStatus; settlementStatus?: SettlementStatus;
  mercadoPagoPaymentId?: string; mercadoPagoPreferenceId?: string; createdAt: string; paidAt?: string; completedAt?: string;
  settledAt?: string; reviewCompletedAt?: string; settlementReason?: 'REVIEW_COMPLETED' | 'MANUAL_SETTLEMENT';
  refundedAt?: string; chargebackAt?: string; cancelledAt?: string;
}
...
export interface VerificationRequest { id: string; userId: string; userName: string; userRole: Role; type: 'IDENTITY' | 'PROFESSIONAL'; documentName: string; documentPath: string; status: VerificationStatus; createdAt: string; }
...
export interface MatchedProfessional { professionalId: string; name: string; professionName: string; avatar: string; matchScore: number; trustScore: number; locationApprox: string; isVerified: boolean; isIdentityVerified?: boolean; isProfessionalVerified?: boolean; rank?: number; rankTag?: string; phoneProtected?: string; matchReasons: string[]; scoreBreakdown?: Record<string, string>; }
export interface RadarOpportunity { id: string; source: string; sourceType: OpportunitySourceType; externalReference?: string; clientUserId?: string; linkedAt?: string; serviceRequestId?: string; convertedAt?: string; category: string; subcategory: string; description: string; city: string; province: string; neighborhood?: string; urgency: OpportunityUrgency; intentScore: number; confidenceScore: number; status: OpportunityStatus; detectedAt: string; lastUpdated: string; assignedOperator?: string; matchedProfessionals: MatchedProfessional[]; matchingStatus?: RadarMatchingStatus; conversionStatus: 'NOT_STARTED' | 'PENDING' | 'CONVERTED' | 'FAILED'; consentStatus: 'PENDING_CONSENT' | 'CONSENT_GRANTED' | 'NOT_REQUIRED'; contactMethod: ContactMethod; notes?: string; environment?: 'simulation' | 'production'; is_test?: boolean; aiAnalysis: { category: string; subcategory: string; intent: OpportunityIntent; urgency: OpportunityUrgency; intentScore: number; confidenceScore: number; spamRiskScore?: number; reasoning: string; recommendedResponseText?: string; }; attribution?: { source: string; campaign?: string; opportunityId?: string; }; }
