...
interface AppContextType {
  ...
  deleteAccount: (userId: string) => Promise<boolean>; acceptQuote: (quoteId: string) => Promise<Transaction | null>; createMercadoPagoCheckout: (transactionId: string) => Promise<string>; connectMercadoPago: () => Promise<void>;
}
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const acceptQuote = async (quoteId: string): Promise<Transaction | null> => {
    ...
    // Acceptance creates a payment obligation; the job is not active until Mercado Pago confirms PAID.
  ...
  const value: AppContextType = { currentUser, setCurrentUser, switchUserRole, switchActiveMode, authLoading, authSessionReady, isAuthPortalOpen, openAuthPortal: () => setIsAuthPortalOpen(true), closeAuthPortal: () => setIsAuthPortalOpen(false), isAdmin, hasRole, users, categories, professions, reviews, requests, quotes, conversations, messages, reports, verifications, notifications, transactions, favorites, betaConfig, inviteCodes, feedbacks, analyticsEvents, radarOpportunities, radarStats, approvalMode, setApprovalMode, toggleFavorite, sharePhoneWithUser, shareAddressWithUser, addReview, submitVerification, deleteAccount, acceptQuote, createMercadoPagoCheckout, connectMercadoPago };
