...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Seed collections asynchronously (only in local dev if empty)
    const seedAll = async () => {
    ...
    try {
      const unsubUsers = onSnapshot(
        ...
        (err) => {
          console.warn('[CONEXA SYNC] Info users listener:', err.message);
        }
      );
      unsubscribers.push(unsubUsers);
    } catch (e) {
      console.warn('[CONEXA SYNC] Error subscribing to users:', e);
    }
    ...
    try {
      const unsubReviews = onSnapshot(
        ...
        (err) => {
          console.warn('[CONEXA SYNC] Info reviews listener:', err.message);
        }
      );
      unsubscribers.push(unsubReviews);
    } catch (e) {
      console.warn('[CONEXA SYNC] Error subscribing to reviews:', e);
    }

    // 3. Service Requests (Open requests / Admin / User requests)
    try {
      const reqRef = collection(db, 'service_requests');
      const unsubRequests = onSnapshot(
        ...
        (err) => {
          console.warn('[CONEXA SYNC] Info requests listener:', err.message);
        }
      );
      unsubscribers.push(unsubRequests);
    } catch (e) {
      console.warn('[CONEXA SYNC] Error subscribing to service_requests:', e);
    }

    // 4. Private / User-scoped Subscriptions (only when signed in)
    if (currentUid) {
      // Quotes
      try {
        const quotesRef = isUserAdmin
          ? collection(db, 'quotes')
          : query(collection(db, 'quotes'), where('clientId', '==', currentUid));
        ...
        const unsubQuotes = onSnapshot(
          ...
          (err) => {
            console.warn('[CONEXA SYNC] Info quotes listener:', err.message);
          }
        );
        unsubscribers.push(unsubQuotes);
      } catch (e) {
        console.warn('[CONEXA SYNC] Error subscribing to quotes:', e);
      }

      // Conversations
      try {
        const convRef = isUserAdmin
          ? collection(db, 'conversations')
          : query(collection(db, 'conversations'), where('participantIds', 'array-contains', currentUid));
        ...
        const unsubConversations = onSnapshot(
          ...
          (err) => {
            console.warn('[CONEXA SYNC] Info conversations listener:', err.message);
          }
        );
        unsubscribers.push(unsubConversations);
      } catch (e) {
        console.warn('[CONEXA SYNC] Error subscribing to conversations:', e);
      }

      // Verifications
      try {
        const verifRef = isUserAdmin
          ? collection(db, 'verifications')
          : query(collection(db, 'verifications'), where('userId', '==', currentUid));
        ...
        const unsubVerifications = onSnapshot(
          ...
          (err) => {
            console.warn('[CONEXA SYNC] Info verifications listener:', err.message);
          }
        );
        unsubscribers.push(unsubVerifications);
      } catch (e) {
        console.warn('[CONEXA SYNC] Error subscribing to verifications:', e);
      }

      // Transactions
      try {
        const transRef = isUserAdmin
          ? collection(db, 'transactions')
          : query(collection(db, 'transactions'), where('clientId', '==', currentUid));
        ...
        const unsubTransactions = onSnapshot(
          ...
          (err) => {
            console.warn('[CONEXA SYNC] Info transactions listener:', err.message);
          }
        );
        unsubscribers.push(unsubTransactions);
      } catch (e) {
        console.warn('[CONEXA SYNC] Error subscribing to transactions:', e);
      }
      ...
      if (isUserAdmin) {
        try {
          const unsubReports = onSnapshot(
            ...
            (err) => {
              console.warn('[CONEXA SYNC] Info reports listener:', err.message);
            }
          );
          unsubscribers.push(unsubReports);
        } catch (e) {
          console.warn('[CONEXA SYNC] Error subscribing to reports:', e);
        }
      }
    }
