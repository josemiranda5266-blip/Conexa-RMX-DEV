...
interface AppContextType {
  currentUser: UserProfile | null; setCurrentUser: (user: UserProfile | null) => void; switchUserRole: (userId: string) => void; switchActiveMode: (mode: 'CLIENT' | 'PROFESSIONAL') => boolean;
  authLoading: boolean; authSessionReady: boolean; isAuthPortalOpen: boolean; openAuthPortal: () => void; closeAuthPortal: () => void; isAdmin: () => boolean; hasRole: (roles: Role[]) => boolean;
  users: UserProfile[]; categories: Category[]; professions: Profession[]; reviews: Review[]; requests: ServiceRequest[]; quotes: Quote[]; conversations: Conversation[]; messages: Record<string, Message[]>;
  reports: UserReport[]; verifications: VerificationRequest[]; notifications: NotificationItem[]; transactions: Transaction[]; favorites: string[]; betaConfig: BetaConfig; inviteCodes: InviteCode[];
  ...
  submitQuote: (quote: Omit<Quote, 'id' | 'createdAt' | 'status'>) => Promise<Quote>; deleteAccount: (userId: string) => Promise<boolean>; startJob: (requestId: string) => Promise<void>; completeJob: (requestId: string) => Promise<void>; acceptQuote: (quoteId: string) => Promise<Transaction | null>; createMercadoPagoCheckout: (transactionId: string) => Promise<string>; connectMercadoPago: () => Promise<void>;
}
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [categories] = useState<Category[]>(INITIAL_CATEGORIES); const [professions] = useState<Profession[]>(INITIAL_PROFESSIONS); const [reviews, setReviews] = useState<Review[]>(INITIAL_REVIEWS);
  const [requests, setRequests] = useState<ServiceRequest[]>(INITIAL_SERVICE_REQUESTS); const [quotes, setQuotes] = useState<Quote[]>(INITIAL_QUOTES); const [conversations] = useState<Conversation[]>(INITIAL_CONVERSATIONS); const [messages] = useState<Record<string, Message[]>>(INITIAL_MESSAGES);
  const [reports, setReports] = useState<UserReport[]>([]); const [verifications, setVerifications] = useState<VerificationRequest[]>([]); const [notifications] = useState<NotificationItem[]>([]); const [transactions, setTransactions] = useState<Transaction[]>([]);
  const [favorites, setFavorites] = useState<string[]>([]); const [betaConfig] = useState<BetaConfig>({} as BetaConfig); const [inviteCodes] = useState<InviteCode[]>([]); const [feedbacks] = useState<FeedbackItem[]>([]); const [analyticsEvents] = useState<AnalyticsEvent[]>([]);
  const [radarOpportunities] = useState<RadarOpportunity[]>(initialRadarOpportunities); const [radarStats] = useState<RadarStats>(initialRadarStats); const [approvalMode, setApprovalMode] = useState<ApprovalMode>('AUTO'); const [authLoading, setAuthLoading] = useState(true); const [authSessionReady, setAuthSessionReady] = useState(false); const [isAuthPortalOpen, setIsAuthPortalOpen] = useState(false);
  ...
  const isAdmin = () => currentUser?.role === 'ADMIN' || currentUser?.role === 'SUPER_ADMIN'; const hasRole = (roles: Role[]) => !!currentUser?.role && roles.includes(currentUser.role);
  ...
  const switchActiveMode = (mode: 'CLIENT' | 'PROFESSIONAL') => { if (!currentUser || (mode === 'PROFESSIONAL' && currentUser.role !== 'PROFESSIONAL')) return false; const updated = { ...currentUser, activeMode: mode }; setCurrentUser(updated); setUsers(list => list.map(u => u.id === currentUser.id ? updated : u)); if (isFirebaseConfigured && db) updateDoc(doc(db, 'users', currentUser.id), { activeMode: mode }).catch(e => console.warn('[CONEXA AUTH] activeMode:', e)); return true; };
  const toggleFavorite = (id: string) => setFavorites(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]); const sharePhoneWithUser = (_conversationId: string, _recipientId: string) => { if (!currentUser) return; }; const shareAddressWithUser = (_conversationId: string, _recipientId: string) => { if (!currentUser) return; };
  const addReview = async (reviewData: Omit<Review, 'id' | 'createdAt' | 'isVerifiedJob'> & { quoteId: string }) => { if (!auth?.currentUser) throw new Error('Debés iniciar sesión para dejar una reseña.'); const token = await auth.currentUser.getIdToken(); const { quoteId, ...payload } = reviewData; const response = await fetch('/api/reviews/create', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ ...payload, quoteId }) }); const data = await response.json().catch(() => ({})); if (!response.ok || !data.success || !data.review) throw new Error(data.error || 'No se pudo guardar la reseña.'); const saved = data.review as Review; setReviews(prev => [saved, ...prev.filter(r => r.id !== saved.id)]); return saved; };
  const submitVerification = async (type: 'IDENTITY' | 'PROFESSIONAL', documentName: string, docUrl: string) => { if (!auth?.currentUser) throw new Error('Debés iniciar sesión para solicitar una verificación.'); const token = await auth.currentUser.getIdToken(); const response = await fetch('/api/verifications/create', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ type, documentName, docUrl }) }); const data = await response.json().catch(() => ({})); if (!response.ok || !data.success || !data.verification) throw new Error(data.error || 'No se pudo enviar la verificación.'); const saved = data.verification as VerificationRequest; setVerifications(prev => [saved, ...prev.filter(v => v.id !== saved.id)]); return saved; };

  const submitQuote = async (quoteData: Omit<Quote, 'id' | 'createdAt' | 'status'>): Promise<Quote> => {
    if (!auth?.currentUser || !currentUser) throw new Error('Debés iniciar sesión para enviar un presupuesto.');
    if (currentUser.role !== 'PROFESSIONAL') throw new Error('Solo un profesional puede enviar presupuestos.');
    if (!quoteData.requestId || !Number.isFinite(Number(quoteData.priceArs)) || Number(quoteData.priceArs) <= 0 || !quoteData.description.trim()) {
      throw new Error('El presupuesto requiere solicitud, precio válido y descripción.');
    }
    const token = await auth.currentUser.getIdToken();
    const response = await fetch('/api/quotes/submit', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ requestId: quoteData.requestId, priceArs: Number(quoteData.priceArs), description: quoteData.description.trim(), materialsIncluded: quoteData.materialsIncluded || '', estimatedTime: quoteData.estimatedTime || '', availableStartDate: quoteData.availableStartDate || '', warrantyInfo: quoteData.warrantyInfo || '', termsAndConditions: quoteData.termsAndConditions || '' })
    });
    const data = await response.json().catch(() => ({}));
    if (!response.ok || !data.success || !data.quote) throw new Error(data.error || 'No se pudo enviar el presupuesto.');
    const saved = data.quote as Quote;
    setQuotes(prev => [saved, ...prev.filter(q => q.id !== saved.id)]);
    setRequests(prev => prev.map(r => r.id === saved.requestId ? { ...r, quotesCount: Number(r.quotesCount || 0) + 1, status: 'QUOTES_RECEIVED' } : r));
    return saved;
  };

  const deleteAccount = async (userId: string) => { if (!auth?.currentUser || userId !== auth.currentUser.uid) return false; try { const token = await auth.currentUser.getIdToken(); const response = await fetch('/api/user/delete-account', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ userId }) }); const data = await response.json(); if (!response.ok || !data.success) return false; await auth.signOut(); setCurrentUser(null); return true; } catch { return false; } };
  ...
  const value: AppContextType = { currentUser, setCurrentUser, switchUserRole, switchActiveMode, authLoading, authSessionReady, isAuthPortalOpen, openAuthPortal: () => setIsAuthPortalOpen(true), closeAuthPortal: () => setIsAuthPortalOpen(false), isAdmin, hasRole, users, categories, professions, reviews, requests, quotes, conversations, messages, reports, verifications, notifications, transactions, favorites, betaConfig, inviteCodes, feedbacks, analyticsEvents, radarOpportunities, radarStats, approvalMode, setApprovalMode, toggleFavorite, sharePhoneWithUser, shareAddressWithUser, addReview, submitVerification, submitQuote, deleteAccount, acceptQuote, startJob, completeJob, createMercadoPagoCheckout, connectMercadoPago };
