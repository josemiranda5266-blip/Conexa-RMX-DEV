...
async function requireAdmin(req: Request, res: Response): Promise<string | null> {
  const auth = await verifyUserAuthToken(req, () => {
    const admin = (req.app as any).locals?.firebaseAdmin;
    return admin || null;
  });
...
chargebackAdminRouter.post('/api/admin/chargebacks/:id/evidence', async (req: Request, res: Response) => {
  ...
  if (!id || files.length < 1 || files.length > 10) return res.status(400).json({ error: 'INVALID_EVIDENCE' });
  ...
  if (normalized.some((file: MPChargebackEvidenceFile) => !file.filename || file.content.byteLength === 0)) return res.status(400).json({ error: 'INVALID_EVIDENCE_FILE' });
  const totalBytes = normalized.reduce((sum: number, file: MPChargebackEvidenceFile) => sum + file.content.byteLength, 0);
...
chargebackAdminRouter.post('/api/admin/chargebacks/:id/submit-evidence', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const paymentTransactionId = String(chargeback.paymentTransactionId || '').trim();
    if (!merchantId || !paymentTransactionId) return res.status(409).json({ error: 'CHARGEBACK_PROVIDER_REFERENCE_MISSING' });
    ...
    const files = Array.isArray(req.body?.files) ? req.body.files : [];
    if (files.length < 1 || files.length > 10) return res.status(400).json({ error: 'INVALID_EVIDENCE' });
    const evidence = files.map((file: any) => ({ filename: String(file.filename || ''), mimeType: file.mimeType, content: Buffer.from(String(file.contentBase64 || ''), 'base64') }));
