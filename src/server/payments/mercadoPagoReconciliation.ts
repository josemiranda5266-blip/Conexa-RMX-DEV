...
function providerStatus(payment: any): ReconciledProviderStatus {
  ...
  const allowed: ReconciledProviderStatus[] = [
...
export async function reconcileMercadoPagoPayment(
  ...
  return db.runTransaction(async tx => {
    ...
    if ((current.mercadoPagoPaymentId && String(current.mercadoPagoPaymentId) !== String(paymentId)) || (current.providerPaymentId && String(current.providerPaymentId) !== String(paymentId))) {
