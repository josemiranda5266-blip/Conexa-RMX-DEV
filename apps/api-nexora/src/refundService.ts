...
export async function requestNexoraRefund(orderId: string, buyerId: string, reason: string) {
  ...
  const prepared = await db.runTransaction(async tx => {
    ...
    tx.update(paymentRef, { refundStatus: 'PROCESSING', refundAmountArs: payment.amountArs, refundReason: reason.trim().slice(0, 500), refundRequestedAt: payment.refundRequestedAt || requestedAt, updatedAt: FieldValue.serverTimestamp() });
  ...
  try {
    ...
    try {
      ...
      // Mercado Pago already accepted the idempotent request. Keep PROCESSING
      // so the signed provider webhook/reconciliation remains authoritative.
    }

    return { accepted: true, paymentTransactionId: prepared.paymentTransactionId, providerRefundId };
  } catch (error) {
    ...
    await db.runTransaction(async tx => {
      ...
      const current = fresh.data() || {};
      if (String(current.status || '').toUpperCase() !== 'REFUNDED') tx.update(paymentRef, { refundStatus: 'FAILED', updatedAt: FieldValue.serverTimestamp() });
    }).catch(() => undefined);
    throw error;
  }
}
