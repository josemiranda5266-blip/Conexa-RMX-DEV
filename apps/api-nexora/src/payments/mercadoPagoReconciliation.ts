...
export async function reconcileMercadoPagoPayment(paymentId: string, connection: MercadoPagoOAuthConnection): Promise<PaymentReconciliationResult> {
  ...
  return db.runTransaction(async tx => {
    ...
    if (resolved.kind === 'NEXORA' && paymentStatus === 'approved') {
      ...
      if (orderStatus === 'PENDING') {
        ...
        listingSnaps.forEach((snap, index) => {
          ...
          if (reservationValid) tx.update(listingRefs[index], { status: stock === 0 ? 'Vendido' : 'Disponible', reservedQuantity: 0, reservedByOrderId: FieldValue.delete(), reservationExpiresAt: FieldValue.delete(), updatedAt: FieldValue.serverTimestamp() });
          else { const restored = stock + Number(items[index].quantity); tx.update(listingRefs[index], { stock: restored, status: restored > 0 ? 'Disponible' : listing.status, reservedQuantity: 0, reservedByOrderId: FieldValue.delete(), reservationExpiresAt: FieldValue.delete(), updatedAt: FieldValue.serverTimestamp() }); }
        });
    ...
    if (transition.refunded) {
      ...
      if (!reversalSnap.exists) tx.create(reversalRef, { id: reversalRef.id, domain: resolved.kind, paymentTransactionId: resolved.ref.id, providerPaymentId: String(paymentId), orderId: String(current.orderId || ''), kind: 'REFUND', amountArs: expectedAmount, currency: 'ARS', reason: reversalReason('REFUND', payment?.status_detail), confirmedAt: now, source: 'MERCADO_PAGO_WEBHOOK' });
    }
