...
import { conversationRepository, listingRepository, orderRepository, paymentRepository, reviewRepository, shopRepository } from './repositories.js';
...
app.post('/api/orders', requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (code === 'LISTING_UNAVAILABLE') return res.status(409).json({ error: code });
    if (code === 'MIXED_SELLER_ORDER') return res.status(409).json({ error: code });
...
app.post('/api/orders/:id/pay', requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const checkout = await prepareNexoraCheckout(req.params.id, req.userId!);
