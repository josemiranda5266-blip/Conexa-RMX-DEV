...
export async function confirmDelivery(orderId: string, buyerId: string) {
  return transitionEscrow(orderId, 'BUYER_CONFIRMED', buyerId);
}
...
export async function openEscrowDispute(orderId: string, buyerId: string, reason: string) {
  ...
  return transitionEscrow(orderId, 'DISPUTE_OPENED', buyerId, reason);
}

export async function autoReleaseEscrow(orderId: string) {
  return transitionEscrow(orderId, 'AUTO_RELEASE');
}

export async function markEscrowRefunded(orderId: string) {
  return transitionEscrow(orderId, 'PAYMENT_REFUNDED');
}
...
export async function expireAndReleaseEligibleEscrows(limit = 100) {
  ...
  const snap = await db.collection(COLLECTION)
    .where('status', '==', 'HELD')
    .where('autoReleaseAt', '<=', cutoff)
    .limit(Math.min(Math.max(limit, 1), 100))
    .get();
  let released = 0;
  for (const doc of snap.docs) {
    try {
      await autoReleaseEscrow(doc.id.replace(/^escrow:/, ''));
      released += 1;
    } catch (error) {
      console.error(`[ESCROW] auto-release failed for ${doc.id}:`, error);
    }
  }
