...
export const nexoraApi = {
  ...
  listOrders() {
    return authenticatedRequest<NexoraOrder[]>('/orders');
  },
  getOrder(id: string) {
    return authenticatedRequest<NexoraOrder>(`/orders/${encodeURIComponent(id)}`);
  },
  createOrder(input: Omit<NexoraOrder, 'id' | 'createdAt' | 'status'>) {
    return authenticatedRequest<NexoraOrder>('/orders', { method: 'POST', body: JSON.stringify(input) });
  },
  completeOrder(id: string) {
    return authenticatedRequest<{ order: NexoraOrder; eventId: string }>(`/orders/${encodeURIComponent(id)}/complete`, { method: 'POST' });
  },
  createReview(input: Omit<NexoraReview, 'id' | 'date'>) {
    return authenticatedRequest<NexoraReview>('/reviews', { method: 'POST', body: JSON.stringify(input) });
  },
};
