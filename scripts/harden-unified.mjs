...
ctx = replaceOnce(ctx, /  const switchActiveMode = \(mode: 'CLIENT' \| 'PROFESSIONAL' \| 'ADMIN'\): boolean => \{[\s\S]*?\n  \};\n\n  const toggleFavorite/, `  const switchActiveMode = (mode: 'CLIENT' | 'PROFESSIONAL'): boolean => {
    ...
    if (mode === 'PROFESSIONAL' && currentUser.role !== 'PROFESSIONAL') return false;
    const updated = { ...currentUser, activeMode: mode };
    setCurrentUser(updated);
    setUsers(uList => uList.map(u => u.id === currentUser.id ? updated : u));
    if (isFirebaseConfigured && db) updateDoc(doc(db, 'users', currentUser.id), { activeMode: mode }).catch(() => {});
    return true;
  };
...
ctx = replaceOnce(ctx, /  const addReview = async \(reviewData:[\s\S]*?\n  \};\n\n  const submitVerification/, `  const addReview = async (reviewData: Omit<Review, 'id' | 'createdAt' | 'isVerifiedJob'>) => {
    ...
    const response = await fetch('/api/reviews/create', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: \`Bearer \${token}\` }, body: JSON.stringify(reviewData) });
    ...
    if (data.professional) setUsers(prev => prev.map(u => u.id === data.professional.id ? { ...u, ...data.professional } : u));
    return savedReview;
  };
...
server = replaceOnce(server, /const effectiveProfessional = auth\.role === 'PROFESSIONAL' \|\| user\.role === 'PROFESSIONAL' \|\| user\.isProfessional === true;\n      if \(!effectiveProfessional\)/, "if (auth.role !== 'PROFESSIONAL')", 'quote professional authorization');
server = replaceOnce(server, /requestId: body\.requestId, professionalId: auth\.userId,/, 'requestId: body.requestId, clientId: request.clientId, professionalId: auth.userId,', 'server-derived quote clientId');
