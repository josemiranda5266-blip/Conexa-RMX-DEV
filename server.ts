...
import { handleProfessionalReviewSave } from "./src/server/reviewRoute.js";
...
app.post("/api/quotes/submit", rateLimiter, async (req: Request, res: Response) => {
  ...
  async function handleVerificationSubmission(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { type, documentName, documentPath } = req.body || {};
      ...
      if (typeof documentName !== 'string' || !documentName.trim() || typeof documentPath !== 'string' || !documentPath.trim()) {
  ...
  app.post('/api/professional-profile/save', rateLimiter, async (req: Request, res: Response) => {
    try {
      const auth = await verifyAuthToken(req);
      if (!auth.isAuthenticated || !auth.userId) {
        res.status(401).json({ success: false, error: 'Se requiere autenticación válida.', code: auth.errorReason || 'UNAUTHORIZED' });
  ...
  app.get('/api/admin/verifications/:verificationId/document-url', rateLimiter, async (req: Request, res: Response) => {
    try {
