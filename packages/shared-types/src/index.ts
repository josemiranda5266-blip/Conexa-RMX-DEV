import type { EventOutboxRecord as CanonicalEventOutboxRecord, NexoraOrderCompletedEvent as CanonicalNexoraOrderCompletedEvent } from '@super-app/shared-events';
...
/** @deprecated Import NexoraOrderCompletedEvent from @super-app/shared-events. */
export type NexoraOrderCompletedEvent = CanonicalNexoraOrderCompletedEvent;
...
export type CheckoutRequestStatus = 'NONE' | 'PROCESSING';
export interface PaymentTransaction { id: string; domain: Domain; orderId: string; buyerId: string; merchantId: string; amountArs: number; currency: 'ARS'; status: PaymentTransactionStatus; provider: 'MERCADO_PAGO'; providerPaymentId?: string; preferenceId?: string; checkoutUrl?: string; checkoutStatus?: CheckoutRequestStatus; checkoutStartedAt?: string; refundStatus?: RefundRequestStatus; refundAmountArs?: number; refundReason?: string; refundRequestedAt?: string; refundConfirmedAt?: string; refundProviderId?: string; paidAt?: string; createdAt: string; updatedAt?: string; refundedAt?: string; chargebackAt?: string; cancelledAt?: string; settlementStatus?: 'PENDING' | 'SETTLED'; settledAt?: string; }
export type EscrowStatus = 'PENDING' | 'HELD' | 'RELEASED' | 'DISPUTED' | 'REFUNDED';
/** @deprecated Use EscrowRecord from @super-app/shared-payments. Kept as a compatibility shape for legacy clients. */
export interface EscrowPayment { id: string; orderId: string; paymentTransactionId?: string; listingId?: string; listingIds?: string[]; amount: number; currency: 'ARS'; buyerId: string; sellerId: string; paymentMethod: 'CARD' | 'MERCADO_PAGO' | 'TRANSFER'; status: EscrowStatus; deliveryPinHash?: string; createdAt: string; heldAt?: string; autoReleaseAt: string; releasedAt?: string; releaseReason?: 'BUYER_CONFIRMED' | 'AUTO_RELEASE' | 'ADMIN_RESOLUTION'; disputeReason?: string; disputedAt?: string; refundedAt?: string; providerPaymentId?: string; custodyMode?: 'PROVIDER_SETTLED_CONTROL'; }
